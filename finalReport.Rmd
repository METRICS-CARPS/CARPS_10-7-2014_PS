...
aov.out <- afex::aov_ez(data=subset(caregiving_salience, tnaythreat=='threat') %>% 
                          rownames_to_column("id"), id = "id", dv = "bias", between = "story")
# BP trying alternative
aov.out <- afex::aov_ez(data=caregiving_salience %>% rownames_to_column("id"), id="id", dv="bias", 
                        between=c("story", "tnaythreat"), check_contrasts=T)
# same result
aov_summary_control <- summary(aov(bias ~ story, data=subset(caregiving_salience, tnaythreat=='control')))
```

```{r reproCheck-simple-effects-aov2}
...
## Step 5: Conclusion
All the reported statistics from Experiment 1 were reproduced **except** one case wasn't very clear. It appears that the authors intended to report an F-statistic (showing the dfs), but there is no statistic and only a p-value that we are unable to reproduce leading to a `DECISION_ERROR`. The particular passage starts on the bottom of page 1382:

All the reported statistics from Experiment 1 were reproduced **except** in one case. 

> F(1, 282), p = .027

The authors responded that this was due to their simple effects anlaysis which relied on a contrast. If we construct a `glm` given their set-up (with an interaction between `story*tnaythreat`) we do get the same p-value of 0.027.
